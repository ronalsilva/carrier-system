...
export function computeCarrierHash(carrier: Record<string, unknown>): string {
    const HASH_FIELDS = [
        "dot_number",
        "legal_name",
        "safety_rating",
...
export function calculateCarrierScore(carrier: {
    ...
    const safetyScore = SAFETY_WEIGHTS[carrier.safety_rating] ?? 0;
    ...
    const authorityScore = AUTHORITY_WEIGHTS[carrier.authority_status] ?? 0;
